    ...
    try:
        ...
        try:
            ...
            game_log_data = await asyncio.wait_for(
                asyncio.to_thread(lambda: PlayerGameLog(player_id=player_id, **api_kwargs).get_dict()), timeout=10.0
            )
...
async def get_season_leaders(season: str = "2024-25") -> SeasonLeadersResponse:
    ...
    try:
        ...
        def create_leaders(
            ...
            is_whole_number: bool = False,
        ) -> SeasonLeadersCategory:
    ...
    try:
        ...
        for player_data in players_data:
            try:
                ...
                logger.warning(
                    f"Error parsing player row (ID: {player_data.get('PERSON_ID', 'unknown')}): {e}. Skipping player."
                )
                continue
