...
class LRUCache:
    ...
    def clear_old_entries(self, max_age_seconds: float) -> int:
        ...
        keys_to_remove = [
            key for key, timestamp in self._timestamps.items() if current_time - timestamp > max_age_seconds
        ]
...
class DataCache:
    ...
    async def _cleanup_finished_games(self):
        ...
        async with self._lock:
            ...
            finished_game_ids = [
                game.gameId for game in scoreboard_data.scoreboard.games if game.gameStatus == 3  # Final
            ]
                ...
                try:
                    ...
                    async with self._lock:
                        ...
                        if scoreboard_data and scoreboard_data.scoreboard:
                            active_games = [
                                game.gameId for game in scoreboard_data.scoreboard.games if game.gameStatus == 2
                            ]
                            ...
                            if finished_games:
                                ...
                                for game_id in finished_games:
                                    self._playbyplay_cache.remove(game_id)
                                logger.info(
                                    f"Immediately cleaned up {len(finished_games)} finished games from play-by-play cache"
                                )

                    logger.debug(
                        f"Scoreboard cache updated: {len(scoreboard_data.scoreboard.games) if scoreboard_data and scoreboard_data.scoreboard else 0} games"
                    )
