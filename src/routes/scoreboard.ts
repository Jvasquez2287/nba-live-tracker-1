...
router.get('/game/:gameId/boxscore', async (req, res) => {
  try {
    ...
    const extractPlayerStats = (leaderData: any, teamTricode: string, teamName: string) => {
...
router.get('/game/:gameId/key-moments', async (req, res) => {
  try {
    ...
    for (let i = 0; i < plays.length; i++) {
      const play = plays[i];
      const homeScore = parseInt((play.score_home as string) || '0');
      const awayScore = parseInt((play.score_away as string) || '0');
