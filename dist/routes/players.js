...
router.get('/season-leaders', async (req, res) => {
    try {
        const statCategory = (req.query.stat || 'PTS').toUpperCase();
        ...
        const statKey = statMap[statCategory] || 'pts';
        const leaders = Array.from(playerStatsMap.values())
            .map(stat => ({
            ...stat,
            statAverage: parseFloat((stat[statKey] / Math.max(stat.games, 1)).toFixed(1))
        }))
...
router.get('/top-by-stat', async (req, res) => {
    try {
        ...
        const topN = Math.min(parseInt(req.query.top_n) || 10, 100);
        const scoreboardData = await dataCache_1.dataCache.getScoreboard();
        const scoreboard = scoreboardData?.scoreboard;
        if (!scoreboard || !scoreboard.games) {
            return res.json({
