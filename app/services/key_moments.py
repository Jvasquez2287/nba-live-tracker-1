...
async def _cleanup_finished_games():
    ...
    try:
        ...
        finished_game_ids = [game.gameId for game in scoreboard_data.scoreboard.games if game.gameStatus == 3]  # Final
        ...
        for game_id in finished_game_ids:
            ...
            moment_ids_to_remove = [
                moment_id for moment_id in _moment_context_cache.keys() if moment_id.startswith(f"{game_id}:")
            ]
        ...
        if removed_moments > 0 or removed_contexts > 0:
            logger.info(
                f"Cleaned up finished games: {len(finished_game_ids)} games, "
        ...
        try:
            ...
            for game_id in list(_key_moments_cache.keys()):
                ...
                _key_moments_cache[game_id] = [
                    m for m in moments if datetime.fromisoformat(m["timestamp"]).timestamp() > cutoff_time
                ]
...
def detect_lead_change(
    ...
    previous_leader = (
        "home"
        if previous_home_score > previous_away_score
        else ("away" if previous_away_score > previous_home_score else None)
    )
    current_leader = (
        "home"
        if current_home_score > current_away_score
        else ("away" if current_away_score > current_home_score else None)
    )
...
async def detect_key_moments(game_id: str) -> List[Dict]:
    ...
    _key_moments_cache[game_id] = [
        m for m in _key_moments_cache[game_id] if datetime.fromisoformat(m["timestamp"]).timestamp() > cutoff_time
    ]
...
async def generate_batched_moment_contexts(moments_with_game_info: List[Dict[str, Any]]) -> Dict[str, str]:
    ...
    if cached_contexts:
        logger.info(
            f"Found {len(cached_contexts)} cached contexts, generating {len(moments_needing_context)} new contexts"
        )
...
async def process_live_games():
    ...
    try:
        ...
        live_games = [game.gameId for game in scoreboard_data.scoreboard.games if game.gameStatus == 2]
        ...
        for game_id in cached_game_ids:
            game = next((g for g in scoreboard_data.scoreboard.games if g.gameId == game_id), None)
